/********************************************************************
* $ID: main.c               Thu 2016-09-29 16:22:45 +0800  lz       *
*                                                                   *
* Description:                                                      *
*                                                                   *
...
void PrefsInit()
{
	...
	gethostname(stPrefs.hostname, sizeof(stPrefs.hostname)); //返回本地主机的标准主机名。
...
int RTSP_full_msg_rcvd(RTSP_buffer *rtsp, int *hdr_len, int *body_len)
{
    ...
    if (rtsp->in_buffer[0] == '$')
    {
        uint16_t *intlvd_len = (uint16_t *)(void*)&rtsp->in_buffer[2];   /*跳过通道标志符*/
...
//	base64_encode2((char *)data, len, psp.base64sps, 512);
	base64_encode((char *)data, psp.base64sps, len);
...
//	base64_encode2((char *)data, len, psp.base64pps, 512);
	base64_encode((char *)data, psp.base64pps, len);
}
