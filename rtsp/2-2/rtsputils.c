/********************************************************************
* $ID: rtsputils.c          Thu 2016-09-29 17:38:16 +0800  lz       *
*                                                                   *
* Description:                                                      *
*                                                                   *
...
extern int test_fist;
extern struct ringbuf ringfifo_first;
				...
				if(sched[i].valid)
				{
					if(!sched[i].rtp_session->pause)
					{
						...
						if((sched[i].rtp_session->hndRtp)&&(s32FindNal))
						{
							...
							if(firstI){
								printf("The pthread %s start  %d \n", __FUNCTION__, ringinfo.size);
								sched[i].play_action((unsigned int)(sched[i].rtp_session->hndRtp), (char *)ringinfo.buffer, ringinfo.size, mnow);
							}
							//有些h264视频文件产生了I帧的数据但是数据格式并不是带有00 00 00 01 67的I数据
							//所以就是发过来的所有buf缓冲区全部发送出去
								sched[i].play_action((unsigned int)(sched[i].rtp_session->hndRtp), (char *)ringinfo.buffer, ringinfo.size, mnow);
						}
					}
				}
