...
HI_S32 SAMPLE_TW2868_CfgV(VIDEO_NORM_E enVideoMode,VI_WORK_MODE_E enWorkMode)
{
    int fd, i;
    int video_mode;
    tw2868_video_norm stVideoMode;
    tw2868_work_mode work_mode;

    int chip_cnt = 2;

    fd = open(TW2868_FILE, O_RDWR);
    if (fd < 0)
    {
        SAMPLE_PRT("open 2868 (%s) fail\n", TW2868_FILE);
        return -1;
    }

    video_mode = (VIDEO_ENCODING_MODE_PAL == enVideoMode) ? tw2868_PAL : tw2868_NTSC ;

    for (i=0; i<chip_cnt; i++)
    {
        stVideoMode.chip    = i;
        stVideoMode.mode    = video_mode;
        if (ioctl(fd, tw2868_SET_VIDEO_NORM, &stVideoMode))
        {
            SAMPLE_PRT("set tw2868(%d) video mode fail\n", i);
            close(fd);
            return -1;
        }
    }
    ...
    for (i=0; i<chip_cnt; i++)
    {
        ...
        if (VI_WORK_MODE_4Multiplex== enWorkMode)
        {
            work_mode.mode = tw2868_4D1_MODE;
        }
        else if (VI_WORK_MODE_2Multiplex== enWorkMode)
        {
            work_mode.mode = tw2868_2D1_MODE;
        }
        else if (VI_WORK_MODE_1Multiplex == enWorkMode)
        {
            work_mode.mode = tw2868_1D1_MODE;
        }
        else
        {
            SAMPLE_PRT("work mode not support\n");
        ...
        ioctl(fd, tw2868_SET_WORK_MODE, &work_mode);
    }
...
HI_S32 SAMPLE_COMM_VI_ADStart(SAMPLE_VI_MODE_E enViMode, VIDEO_NORM_E enNorm)
{
    ...
    switch (enViMode)
    {
		...
		case SAMPLE_VI_MODE_16_D1Cif:
            ...
            //s32Ret = SAMPLE_TW2865_CfgV(enNorm, enWorkMode);
            break;
			s32Ret = SAMPLE_TW2868_CfgV(enNorm, enWorkMode);
            if (s32Ret != HI_SUCCESS)
            {
                SAMPLE_PRT("SAMPLE_TW2865_CfgV failed with %#x!\n",\
                        s32Ret);
...
HI_S32 SAMPLE_COMM_VI_Start(SAMPLE_VI_MODE_E enViMode, VIDEO_NORM_E enNorm)
{
    ...
    for(i=0; i<stViParam.s32ViChnCnt; i++)
    {
		...
		/*if( enViMode == SAMPLE_VI_MODE_8_D1 && i==4 )
		{
			ViChn = 8;	
		}
		else if( enViMode == SAMPLE_VI_MODE_8_D1 && i==5 )
		{
			ViChn = 9;
		}
        else if( enViMode == SAMPLE_VI_MODE_8_D1 && i==6 )
		{
			ViChn = 10;
		}
		else if( enViMode == SAMPLE_VI_MODE_8_D1 && i==7 )
		{
			ViChn = 11;
		}*/
		//printf("MyViChn=%d\r\n",ViChn);
