...
struct profileid_sps_pps psp[CH_MAX]; 

StServPrefs stPrefs;
int g_s32Maxfd = 0;
int g_s32DoPlay[CH_MAX] = {0,0,0,0, 0,0,0,0};

uint32_t s_u32StartPort=RTP_DEFAULT_PORT;
uint32_t s_uPortPool[CH_MAX][MAX_CONNECTION];
int g_s32Quit = 0;
void RTP_port_pool_init(int ch,int port);
...
**
**************************************************************************************************/
int RTP_get_port_pair(int ch,port_pair *pair)
{
    int i;

    for (i=0; i<MAX_CONNECTION; ++i)
    {
        if (s_uPortPool[ch][i]!=0)
        {
            pair->RTP=(s_uPortPool[ch][i]-s_u32StartPort)*2+s_u32StartPort;
...
int RTSP_full_msg_rcvd(RTSP_buffer *rtsp, int *hdr_len, int *body_len)
{
    ...
    if (rtsp->in_buffer[0] == '$')
    {
    	uint16_t *intlvd_len = (uint16_t *)(void*)&rtsp->in_buffer[2];  
...
void GetSdpDescr(RTSP_buffer * pRtsp, char *pDescr, char *s8Str)
{
	...
	strcat(pDescr, inet_ntoa(((struct sockaddr_in *)(void*)(&pRtsp->stClientAddr))->sin_addr));
	strcat(pDescr, "\r\n");
	...
	sprintf(rtp_port,"%d",pRtsp->ch*2+RTP_DEFAULT_PORT);
	...
	strcat(pDescr,"b=RR:0\r\n");
		...
		strcat(pDescr,psp[pRtsp->ch].base64profileid);
		strcat(pDescr,";sprop-parameter-sets=");
		strcat(pDescr,psp[pRtsp->ch].base64sps);
		strcat(pDescr,",");
		strcat(pDescr,psp[pRtsp->ch].base64pps);
		strcat(pDescr,";");
		strcat(pDescr, "\r\n");
		strcat(pDescr,"a=control:trackID=0");
		strcat(pDescr, "\r\n");

printf("\n\n%s,%d===>psp.base64profileid=%s,psp.base64sps=%s,psp.base64pps=%s\n\n",__FUNCTION__,__LINE__,psp[pRtsp->ch].base64profileid,psp[pRtsp->ch].base64sps,psp[pRtsp->ch].base64pps);
...
int RTSP_describe(RTSP_buffer * pRtsp)
{
	...
	if(memcmp(object,"ch0",3)==0)
	{
		pRtsp->ch=0;
	}
	else if(memcmp(object,"ch1",3)==0)
	{
		pRtsp->ch=1;
	}
	else if(memcmp(object,"ch2",3)==0)
	{
		pRtsp->ch=2;
	}
	else if(memcmp(object,"ch3",3)==0)
	{
		pRtsp->ch=3;
	}
	else if(memcmp(object,"ch4",3)==0)
	{
		pRtsp->ch=4;
	}
	else if(memcmp(object,"ch5",3)==0)
	{
		pRtsp->ch=5;
	}
	else if(memcmp(object,"ch6",3)==0)
	{
		pRtsp->ch=6;
	}
	else if(memcmp(object,"ch7",3)==0)
	{
		pRtsp->ch=7;
	}
	else
...
int RTSP_setup(RTSP_buffer * pRtsp)
{
	...
	if((pStr = strstr(s8TranStr, RTSP_RTP_AVP)))
	{
		...
		if ( !*pStr || (*pStr == ';') || (*pStr == ' '))
		{
			...
			if (strstr(s8TranStr, "unicast"))
			{
				...
				if (RTP_get_port_pair(pRtsp->ch,&Transport.u.udp.ser_ports) != ERR_NOERROR)
				{
				...
				rtp_s->hndRtp = (struct _tagStRtpHandle*)RtpCreate((unsigned int)(((struct sockaddr_in *)(void*)(&pRtsp->stClientAddr))->sin_addr.s_addr), Transport.u.udp.cli_ports.RTP, _h264nalu);
				printf("<><><><>Creat RTP<><><><>\n");
	...
	pRtsp->session_list->rtp_session->sched_id = schedule_add(pRtsp->ch,rtp_s);
		...
		if (pRtspSesn->session_id == s32SessionId)
		{
			...
			for (pRtpSesn = pRtspSesn->rtp_session; pRtpSesn != NULL; pRtpSesn = pRtpSesn->next)
			{
				...
				if (!pRtpSesn->started)
				{
					...
					printf("\t+++++++++++++++++++++\n");
					if(pRtpSesn->sched_id == 0){
					if (schedule_start(pRtsp->ch,pRtpSesn->sched_id, NULL) == ERR_ALLOC)
					{
						return ERR_ALLOC;
					}
					g_s32DoPlay[pRtsp->ch]++;
					}
					else{
						printf("error not add now ch \n");
						send_reply(400, 0, pRtsp);
						return ERR_ALLOC;
					}
				}
...
int RTSP_teardown(RTSP_buffer * pRtsp)
{
	...
	while (pRtpSesn != NULL)
	{
		...
		schedule_remove(pRtsp->ch,pRtpSesnTemp->sched_id);
		g_s32DoPlay[pRtsp->ch]--;

	}
	if (g_s32DoPlay[pRtsp->ch] == 0) 
	{
		printf("no user online now resetfifo\n");
		ringreset(pRtsp->ch);
		RTP_port_pool_init(pRtsp->ch,RTP_DEFAULT_PORT);
	}
            ...
            if (res==ERR_CONNECTION_CLOSE || res==ERR_GENERIC)
            {
                ...
                if (pRtsp->session_list!=NULL)
                {
                    ...
                    while (r!=NULL)
                    {
                        ...
                        schedule_remove(pRtsp->ch,r->sched_id);
                        r=t;
                    }
                ...
                close(pRtsp->fd);
                --*conn_count;
				printf("%s,%s,%d pRtsp->cH=%d\n",__FILE__,__FUNCTION__,__LINE__,pRtsp->ch);
...
void RTP_port_pool_init(int ch,int port)
{
    ...
    for (i=0; i<MAX_CONNECTION; ++i)
    {
    	s_uPortPool[ch][i] = i+s_u32StartPort+MAX_CONNECTION*ch;
    }
...
void EventLoop(int s32MainFd)
{
	...
	if (s32Fd >= 0)
	{
		...
		if (!u32FdFound)
		{
			...
			if (s32ConCnt<MAX_CONNECTION*CH_MAX)
			{
				++s32ConCnt;
...
void UpdateSps(int ch,unsigned char *data,int len)
{
	...
	sprintf(psp[ch].base64profileid,"%x%x%x",data[1],data[2],data[3]);//sps[0] 0x67

	base64_encode2((char *)data, len, psp[ch].base64sps, 512);
...
void UpdatePps(int ch,unsigned char *data,int len)
{
	...
	base64_encode2((char *)data, len, psp[ch].base64pps, 512);
}
